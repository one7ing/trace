...
public class InterviewServiceImpl implements InterviewService {
    ...
    public Map<String, Object> startInterview(Long userId, InterviewStartRequest request) {
        ...
        session.put("userId", userId); session.put("industry", request.getIndustry());
        session.put("skills", request.getSkills()); session.put("totalQuestions", request.getQuestionCount());
        session.put("currentQuestion", 0); session.put("questions", new ArrayList<Map<String, Object>>());
        ...
        return Map.of("sessionId", sessionId, "question", fq.get("question"), "currentQuestion", 1, "totalQuestions", request.getQuestionCount(), "status", "IN_PROGRESS");
    }
    ...
    private Map<String, Object> generateQuestion(String industry, List<String> skills) {
        String q = chatClientBuilder.build().prompt().system(SYSTEM_PROMPT).user(String.format("为%s行业%s出一题。只输出题目。", industry, String.join("、", skills))).call().content();
        Map<String, Object> m = new HashMap<>(); m.put("question", q); return m;
    }

    private Map<String, Object> evaluateAnswer(String industry, List<String> skills, String question, String answer) {
        try {
            String resp = chatClientBuilder.build().prompt().system(SYSTEM_PROMPT).user(String.format("评估面试：\n行业：%s\n技能：%s\n题：%s\n答：%s\n输出JSON：{\"score\":8,\"comment\":\"点评\"}", industry, String.join("、", skills), question, answer)).call().content();
            Map<String, Object> m = new HashMap<>();
            m.put("score", new BigDecimal(extractJson(resp, "score")));
            m.put("comment", extractJson(resp, "comment"));
            return m;
        } catch (Exception e) { return Map.of("score", new BigDecimal("5"), "comment", "评分暂不可用"); }
    }
    ...
    private Long saveRecord(Map<String, Object> s, BigDecimal avg) {
    ...
    @Override @SuppressWarnings("unchecked") public Flux<String> submitAnswerStream(String sid, InterviewAnswerRequest req) { Map<String, Object> r = submitAnswer(sid, req); return Flux.just(r.toString()); }
